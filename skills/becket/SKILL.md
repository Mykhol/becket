...
- `becket gc [--apply] [--idle-days N] [--deps-idle-days M]` — remove merged,
  ...
  one with uncommitted or unpushed work, or one holding unknown files at the
  workspace root. `.reports/` and `docs/` are copied to
  `<platform>/.becket/archive/<id>/` before removal. Commits GitHub still
  holds (squash-merged or PR-head checkouts) count as pushed.
